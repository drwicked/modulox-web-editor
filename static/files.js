...
function onSavePreset(e){
	...
	if(!module_placed) document.getElementById("Set_text").innerHTML = "Placer tous les modules avant de sauvegarder";
	else
	{
...
function openFileEntry(fileEntry) {
   ...
   reader.onload = function(e) {
	   ...
	   if(module_number != j) document.getElementById("Set_text").innerHTML = "Il vous faut " + j + " modules pour charger ce patch. Vous en avez connectés " + module_number + ".";
	   else
	   {
		   document.getElementById("Set_text").innerHTML = "Preset chargé. Déplacez les composants sur la grille avec un drag-and-drop ou modifiez les paramètres dans l'onglet MIDI";
		   //moduleList();
		   module_placed = 1;
		   drawModules();
	   }
